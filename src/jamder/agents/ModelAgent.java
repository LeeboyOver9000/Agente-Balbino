...
public abstract class ModelAgent extends ReflexAgent
{
	...
	private Norm getTempNorm(String key)
	{
		return restrictTempNorms.get(key);
	}

	private boolean containTempNorm(String key)
	{
		return restrictTempNorms.containsKey(key);
	}
		...
		if( containBelief( belief.getName() ) )
		{
			...
			if( norm != null )
			{
				...
				if(actionList != null && !actionList.isEmpty())
				{
					...
					{
						Action temp = it.next();
						if( temp.getExecutionPrice() < cheapest.getExecutionPrice() )
							cheapest = temp;
					}
